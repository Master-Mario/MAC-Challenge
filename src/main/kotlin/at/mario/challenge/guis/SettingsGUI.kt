...
object SettingsGUI {
    ...
    fun open(player: HumanEntity, page: Int = 0) {
        ...
        if (Config().config.getBoolean(Config.Companion.Keys.SETTINGS_FREEZE_ON_PAUSE)) {
            settings.add(Utils().createItem(
        ...
        settings.add(Utils().createItem(
            ...
            lines = listOf(
                ...
                cmp(Lang.translate("current"), KColors.GRAY) + cmp(Config().config.getString(Config.Companion.Keys.LANGUAGE) ?: "en", KColors.LIGHTGREEN)
            )
        ))
