    ...
    val onPunch = listen<EntityDamageByEntityEvent> {
        if (it.damageSource.causingEntity is Player){
            if (Challenges.SEQUENCE.active){
                ...
                if (Config().config.getString("sequence.${Config().config.getString("sequence.next")!!.toInt()}") != player.name){
                    Bukkit.broadcast(cmp(Config().config.getString("sequence.${Config().config.getString("sequence.next")!!.toInt()}")!!))
                    it.isCancelled = true
                    player.sendMessage(Main.prefix + cmp("${KColors.RED}Du bist nicht der Nächste!"))
                    player.health = 0.0
                    Bukkit.getOnlinePlayers().forEach { player ->
                        val sound = Sound.sound(Key.key("block.anvil.land"), Sound.Source.MASTER, 0.5f, 1f)
                        player.playSound(sound)
                    }
                    if (Bukkit.getOnlinePlayers().size+1 == Config().config.getString("sequence.next")!!.toInt()){
                        Config().addInt("sequenz.next", 1)
                    }else{
                        Config().addInt("sequenz.next", Config().config.getString("sequenz.next")!!.toInt() + 1)
                    }
                }else{
                    Bukkit.getOnlinePlayers().forEach { player ->
